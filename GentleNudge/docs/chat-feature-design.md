...
- Add `Constants.chatModel` **as a user-settable value** (Settings picker), choices: **`claude-opus-4-8`** (default) and **`claude-sonnet-5`** — both verified current against the `claude-api` reference (Opus 4.8 = $5/$25 per MTok; Sonnet 5 = $3/$15, $2/$10 intro through 2026-08-31). Add a **reasoning/effort** control exposed as a setting: `output_config.effort` ∈ `low`/`medium`/`high`/`xhigh`/`max` (default `high`), plus a thinking toggle (`thinking: {type:"adaptive"}` vs. off). Persist via `@AppStorage`.
- **Independently, fix the enhance feature now:** `Constants.claudeModel = "claude-sonnet-4-20250514"` is **retired (2026-06-15)** and is likely already failing. Point it at **`claude-sonnet-5`** (the official replacement) or `claude-opus-4-8`.
- **Request params** (verified against `claude-api`): `max_tokens` modest (e.g. 4096 — but **measure** worst-case multi-create + self-repair turns; raise if truncation appears). Omit `temperature`/`top_p`/`top_k` (non-default values return 400 on both Opus 4.8 and Sonnet 5). **Thinking default differs by model** — omitting `thinking` runs *without* thinking on Opus 4.8 but runs *adaptive* on Sonnet 5, so set it explicitly (`{type:"disabled"}` for latency, or `{type:"adaptive"}` if date math proves shaky) to get consistent behavior across the switchable model setting. Never send legacy `{type:"enabled", budget_tokens:N}` — it 400s on both.
...
> **Strict-mode caveat (verified against `claude-api`):** strict mode supports basic types (incl. `null`), `enum`, `const`, `anyOf`, `allOf`, `$ref`/`$def`, and requires `additionalProperties:false`. It does **not** support **numerical constraints** (`minimum`/`maximum`) or **string constraints** (`minLength`/`maxLength`). Consequence for the schemas below: **do not encode the `recurrence_anchor_day` 1–31 range as `minimum`/`maximum`** — express it in the description and enforce it in the executor. The nullable-union spelling (`["type","null"]`) is the one genuinely-**UNVERIFIED** shape; if a live schema check rejects it, switch those fields to `anyOf: [{type:…}, {type:"null"}]` (supported) rather than dropping strict mode.
...
- **UI-event notes (Phase 1, not Phase 2):** when the user taps Undo/Delete or edits a card between turns, deliver a **structured** note next turn (`event type + UUID + new status` — **never** raw titles, which are an injection vector). **Portability constraint (verified):** mid-conversation `{"role":"system"}` messages are **Opus-4.8-only and 400 on Sonnet 5**. Because model is a switchable setting, the **default channel is a delimited `<ui-event>` data block inside the next user message** (works on both models); only use the `system`-role channel when the selected model is Opus 4.8. Either way it's placement-sensitive: the note must follow a user or tool-result turn and never sit between a `tool_use` and its `tool_result`.
...
Layout: `tools` (byte-stable) → static system blocks with a `cache_control:{type:"ephemeral"}` breakpoint on the last static block → dynamic tail → messages. **Verified against `claude-api`:** the minimum cacheable prefix on **Opus 4.8 is 4,096 tokens** (Sonnet 5 ≈ 2,048). So a ~1.5–2.5K static prefix **silently will not cache on Opus 4.8** — no error, just `cache_creation_input_tokens: 0`. (v1 had this right at 4,096; the codex review's "1,024" was wrong for this model.) Implication: either accept no caching at v1's prompt size, or grow the cached prefix past 4K (e.g. fold the tool schemas + full behavior policy into the cached block) before relying on it. The layout costs nothing and future-proofs prompt growth, but **do not count on cache savings in cost estimates**; verify with `usage.cache_read_input_tokens` during development.
